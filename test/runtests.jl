...
function solveandcheck(
    ...
    if !in(lscachetype, (Hypatia.Chol2, Hypatia.Naive3, Hypatia.Naive4, Hypatia.QRChol))
        error("linear system solver method $lscachetype not recognized")
    end
    if !preprocess && lscachetype == Hypatia.QRChol
        error("must use preprocessing with QRChol linear system solver method")
    end
    ...
    if preprocess
        if lscachetype == Hypatia.QRChol
            ...
            L = lscachetype(P1, c1, A1, b1, G1, h, cone, Q2, RiQ1)
        else
            (P1, c1, A1, b1, G1, prkeep, dukeep, _, _) = Hypatia.preprocess_data(P, c, A, b, G, useQR=false)
            L = lscachetype(P1, c1, A1, b1, G1, h, cone)
        end
        Hypatia.load_data!(mdl, P1, c1, A1, b1, G1, h, cone, L)
    else
        prkeep = 1:length(b)
        dukeep = 1:length(c)
        L = lscachetype(P, c, A, b, G, h, cone)
...
solveandcheck(
    ...
    atol = 1e-4,
    rtol = 1e-4,
    ) = solveandcheck(mdl, Symmetric(spzeros(length(c), length(c))), c, A, b, G, h, cone, lscachetype, atol=atol, rtol=rtol)

...
function _shapeconregr1_JuMP()
    ...
    (X, y) = generateregrdata(f, -1.0, 1.0, n, npoints, signal_ratio=signal_ratio, use_leastsqobj=false)
    (mdl, p) = build_shapeconregr_WSOS(X, y, deg, ShapeData(n))
    truemin = 4.4065e1
    solveandcheck_JuMP(mdl, truemin)
end
...
function _shapeconregr6_JuMP()
    (n, deg, npoints, signal_ratio, f) = (2, 3, 100, 0.0, x -> exp(norm(x)))
    (X, y) = generateregrdata(f, -1.0, 1.0, n, npoints, signal_ratio=signal_ratio, use_leastsqobj=true)
    (mdl, p) = build_shapeconregr_WSOS(X, y, deg, ShapeData(n))
    truemin = 5.4584e0
    solveandcheck_JuMP(mdl, truemin)
end

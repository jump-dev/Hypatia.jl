...
function test_barrier_oracles(
    ...
    scale::Real = 10000,
    tol::Real = 100eps(T),
    init_point_tol::Real = T(1e-2),
    init_point_only::Bool = false,
    ) where {T <: Real}
    CO.setup_data(cone)
...
function test_orthant_barrier(T::Type{<:Real}; init_point_only::Bool = false, dim_range = [1, 3])
    barrier = s -> -sum(log, s)
    ...
    for dim in [1, 3]
        ...
        test_barrier_oracles(cone, barrier, init_point_tol = sqrt(eps(T)), init_point_only = init_point_only)
    end
    return
end

function test_epinorminf_barrier(T::Type{<:Real}; init_point_only::Bool = false, dim_range = [2, 4])
    function barrier(s)
        u = s[1]
        w = s[2:end]
...
function test_epinormeucl_barrier(T::Type{<:Real}; init_point_only::Bool = false, dim_range = [2, 4])
    function barrier(s)
        u = s[1]
        w = s[2:end]
...
function test_epipersquare_barrier(T::Type{<:Real}; init_point_only::Bool = false, dim_range = [3, 5])
    function barrier(s)
        u = s[1]
        v = s[2]
        w = s[3:end]
...
function test_hypoperlog_barrier(T::Type{<:Real}; init_point_only::Bool = false, dim_range = [3, 5])
    function barrier(s)
        u = s[1]
        v = s[2]
        w = s[3:end]
    ...
    for dim in dim_range
        ...
        test_barrier_oracles(cone, barrier, init_point_tol = T(1e-2), init_point_only = init_point_only)
    end
    return
end
...
function test_power_barrier(T::Type{<:Real}; init_point_only::Bool = false, m_range = [2, 4], n_range = [1, 3])
    Random.seed!(1)
...
function test_hypogeomean_barrier(T::Type{<:Real}; init_point_only::Bool = false, dim_range = [2, 4])
    Random.seed!(1)
    for dim in dim_range
...
function test_epinormspectral_barrier(T::Type{<:Real}; init_point_only::Bool = false, nm_range = [(1, 2), (2, 2), (2, 3)])
    for (n, m) in nm_range
...
function test_possemideftri_barrier(T::Type{<:Real}; init_point_only::Bool = false, side_range = [1, 2, 3])
    for side in side_range
        ...
        test_barrier_oracles(cone, R_barrier, init_point_tol = sqrt(eps(T)), init_point_only = init_point_only)
        ...
        test_barrier_oracles(cone, C_barrier, init_point_tol = sqrt(eps(T)), init_point_only = init_point_only)
    end
    return
end

function test_hypoperlogdettri_barrier(T::Type{<:Real}; init_point_only::Bool = false, side_range = [1, 2, 3])
    for side in side_range
        ...
        test_barrier_oracles(cone, barrier, init_point_tol = T(0.1), init_point_only = init_point_only)
    end
    return
end

function test_wsospolyinterp_barrier(T::Type{<:Real}; init_point_only::Bool = false, n_range = [1, 2, 3], halfdeg_range = [1, 2])
    Random.seed!(1)
    for n in n_range, halfdeg in halfdeg_range
        ...
        test_barrier_oracles(cone, barrier, init_point_tol = Inf, init_point_only = init_point_only)
    end

...
function test_oracles(
    ...
    @test Cones.get_proxsqr(cone, one(T), false) <= 1 # max proximity
    @test Cones.get_proxsqr(cone, one(T), true) <= dim # sum proximity
...
function show_time_alloc(
    ...
    println("get_proxsqr")
    @time Cones.get_proxsqr(cone, one(T), false)

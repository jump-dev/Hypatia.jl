...
# what we want
@constraint(model, [Hp[i,j](pts[u, :]) for i in 1:n for j in 1:i for u in 1:U] in wsos_mat_cone)
...
@constraint(model, [u in 1:U, i in 1:n, j in 1:i], Hp[i,j](pts[u,:]) == 12 * pts[u,i]^2 * Float64(i == j))

...
function solveandcheck_JuMP(mdl, truemin; atol=1e-4, rtol=1e-4)
    JuMP.optimize!(mdl)
    ...
    @test pobj ≈ truemin atol=atol rtol=rtol
end
...
function namedpoly10_JuMP()
    ...
    # WSOS formulation
    mdl = build_JuMP_namedpoly_WSOS(x, f, dom, d=5)
    solveandcheck_JuMP(mdl, truemin, atol=1e-3)
    # SDP formulation
    # mdl = build_JuMP_namedpoly_PSD(x, f, dom, d=5)
    # solveandcheck_JuMP(mdl, truemin, atol=1e-3)
end

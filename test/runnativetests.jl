...
for inst_name in inst_all, (stepper, real_types) in steppers, T in real_types
    test_info = "$inst_name $T $(string_nameof(stepper))"
...
for inst_name in inst_all, (system_solver, real_types) in system_solvers, T in real_types
    test_info = "$inst_name $T $(string_nameof(system_solver))"
    options = (; common_options..., system_solver = system_solver{T}())
    test_instance_solver(inst_name, T, options, test_info)
end
end

@testset "indirect initialization tests" begin
for inst_name in inst_cones_few, T in all_reals
    test_info = "$inst_name $T"
...
for inst_name in inst_preproc, T in all_reals
    test_info = "$inst_name $T"
    options = (; common_options..., preprocess = true, reduce = false, system_solver = Solvers.SymIndefDenseSystemSolver{T}())
    test_instance_solver(inst_name, T, options, test_info)
end
end

@testset "no reduce tests" begin
for inst_name in inst_cones_few, T in all_reals
    test_info = "$inst_name $T"

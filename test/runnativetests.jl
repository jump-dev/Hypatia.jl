...
all_reals = [Float32, Float64, BigFloat]
diff_reals = [Float64, BigFloat]
...
@testset "native tests" begin
    @testset "default options tests" begin
        ...
        inst_defaults = vcat(inst_preproc, inst_infeas, inst_cones_many)
    ...
    @testset "system solvers tests" begin
        ...
        for inst_name in inst_minimal,
            (syssolver, real_types) in syssolvers,
            T in real_types

            options = (; default_options..., syssolver = syssolver{T}(), reduce = false)
    ...
    @testset "model modification tests" begin
        ...
        for inst_name in inst_modify,
            (syssolver, real_types) in syssolvers,
            T in real_types,
            rescale in (false, true),
            preprocess in (false, true),

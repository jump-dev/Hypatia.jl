...
function portfolio(
    ...
    if :l1 in risk_measures && use_l1ball
        add_single_ball(CO.EpiNormInf{Float64}(num_stocks + 1, true), gamma)
    end
    if :linf in risk_measures && use_linfball
        add_single_ball(CO.EpiNormInf{Float64}(num_stocks + 1), gamma / num_stocks)
    end
    ...
    if :linf in risk_measures && !use_linfball
        ...
        b = vcat(b, gamma / num_stocks * ones(2 * num_stocks))
        G = [
            G zeros(size(G, 1), 2 * num_stocks)
...
function test_portfolio(instance::Function; options, rseed::Int = 1)
    ...
    @show sum(abs, d.sigma_half * r.x[1:size(d.sigma_half, 1)])

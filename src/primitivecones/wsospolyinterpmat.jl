...
mutable struct WSOSPolyInterpMat <: PrimitiveCone
    ...
    function WSOSPolyInterpMat(r::Int, u::Int, ipwt::Vector{Matrix{Float64}}, isdual::Bool)
        ...
        prmtv.barfun = (x -> -logdet(makemat(x, ipwt, r, u)))
        prmtv.diffres = DiffResults.HessianResult(prmtv.g)
        return prmtv
    end
end

function makemat(matpnt, ipwt::Vector{Matrix{Float64}}, r::Int, u::Int)
    # mat .= 0.0
    mat = similar(matpnt, u*r, u*r)
    mat .= 0.0
    for ipwtj in ipwt
        l = size(ipwtj, 2)
...
barrierpar_prmtv(prmtv::WSOSPolyInterpMat) = sum(size(ipwtj, 2) for ipwtj in prmtv.ipwt)
getintdir_prmtv!(arr::AbstractVector{Float64}, prmtv::WSOSPolyInterpMat) = (@. arr = 1.0; arr)
...
function incone_prmtv(prmtv::WSOSPolyInterpMat, scal::Float64)
    ...
    # TODO don't form this matrix twice!!
    ...
    # return issuccess(bunchkaufman!(Symmetric(prmtv.mat, :u), true, check=false))
    return issuccess(bunchkaufman!(Symmetric(prmtv.mat, :U), true, check=false))
end

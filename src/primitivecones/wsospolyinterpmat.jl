...
mutable struct WSOSPolyInterpMat <: PrimitiveCone
    ...
    function WSOSPolyInterpMat(r::Int, u::Int, ipwt::Vector{Matrix{Float64}}, isdual::Bool)
        ...
        prmtv.barfun = (x -> barfun(x, ipwt, u))
...
function barfun(matpnt, ipwt::Vector{Matrix{Float64}}, u::Int)
    ret = 0.0
    for ipwtj in ipwt
        l = size(ipwtj, 2)
        ...
        for j in 1:l, i in 1:l
            mat[i,j] = sum(sum(ipwtj[ui,i] * ipwtj[ui,j] * matpnt[:,:,ui]) for ui in 1:u)
...
function inconefun(matpnt, ipwt::Vector{Matrix{Float64}}, u::Int)
    ret = true
    for ipwtj in ipwt
        l = size(ipwtj, 2)
        # mat = zeros(l*r, l*r)
...
function incone_prmtv(prmtv::WSOSPolyInterpMat, scal::Float64)
    ...
    if !inconefun(prmtv.matpnt, prmtv.ipwt, prmtv.u)
        return false
    end

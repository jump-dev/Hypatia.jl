...
mutable struct NaiveElimPardisoCombinedHSDSystemSolver{T <: Real} <: CombinedHSDSystemSolver{T}
    use_sparse::Bool
    ...
    function NaiveElimPardisoCombinedHSDSystemSolver{T}(model::Models.LinearModel{T}; use_sparse::Bool = false) where {T <: Real}
...
function get_combined_directions(solver::HSDSolver{T}, system_solver::NaiveElimPardisoCombinedHSDSystemSolver{T}) where {T <: Real}
    model = solver.model
    (n, p, q) = (model.n, model.p, model.q)
    cones = model.cones

...
mutable struct WSOSMonoNonnegative{T <: Real, R <: RealOrComplex{T}} <: Cone{T}
    use_dual::Bool
    dim::Int
    Ps::Vector{Matrix{R}}
    point::Vector{T}
    ...
    function WSOSMonoNonnegative{T, R}(
        U::Int,
        Ps::Vector{Matrix{R}},
        is_dual::Bool;
...
WSOSMonoNonnegative{T, R}(dim::Int, Ps::Vector{Matrix{R}}) where {R <: RealOrComplex{T}} where {T <: Real} = WSOSMonoNonnegative{T, R}(dim, Ps, false)

function setup_data(cone::WSOSMonoNonnegative{T, R}) where {R <: RealOrComplex{T}} where {T <: Real}
    reset_data(cone)
    dim = cone.dim
...
get_nu(cone::WSOSMonoNonnegative) = sum(size(Pk, 2) for Pk in cone.Ps)

# TODO find "central" initial point, like for other cones
set_initial_point(arr::AbstractVector, cone::WSOSMonoNonnegative) = (arr .= 1)

# TODO order the k indices so that fastest and most recently infeasible k are first
# TODO can be done in parallel
function update_feas(cone::WSOSMonoNonnegative)
...
function update_grad(cone::WSOSMonoNonnegative)
    @assert cone.is_feas
...
function update_hess(cone::WSOSMonoNonnegative)

...
mutable struct WSOSPolyInterpRSOC <: Cone
    use_dual::Bool
    dim::Int
    R::Int
    U::Int
    ...
    function WSOSPolyInterpRSOC(R::Int, U::Int, rotated::Bool, ipwt::Vector{Matrix{Float64}}, is_dual::Bool)
        for ipwtj in ipwt
            @assert size(ipwtj, 1) == U
        end
        cone = new()
        cone.use_dual = !is_dual # using dual barrier
        dim = U * (R + 2) # TODO inconsistent with SOC, I think this is more convenient
        cone.dim = dim
        cone.R = R
        cone.U = U
        cone.rotated = rotated
        cone.ipwt = ipwt
        ...
        cone.li_lambda = [Vector{Matrix{Float64}}(undef, R + 1) for ipwtj in ipwt] # FIXME should be R
        for j in eachindex(ipwt), r in 1:(R + 1)
            cone.li_lambda[j][r] = similar(ipwt[1], size(ipwt[j], 2), size(ipwt[j], 2))
        end
        cone.PlambdaiP = [Vector{Vector{Matrix{Float64}}}(undef, R + 1) for ipwtj in ipwt] # matrix is (R + 1)*(R + 1)
        for j in eachindex(ipwt), r1 in 1:(R + 1)
...
WSOSPolyInterpRSOC(R::Int, U::Int, rotated::Bool, ipwt::Vector{Matrix{Float64}}) = WSOSPolyInterpRSOC(R, U, rotated, ipwt, false)

get_nu(cone::WSOSPolyInterpRSOC) = sum(size(ipwtj, 2) for ipwtj in cone.ipwt)

function set_initial_point(arr::AbstractVector{Float64}, cone::WSOSPolyInterpRSOC)
    arr .= 0.0
    arr[1:(2 * cone.U)] .= 1.0
    return arr
end

function check_in_cone(cone::WSOSPolyInterpRSOC)
    ...
    for j in eachindex(cone.ipwt)
        ...
        # first lambda
        # point_pq = 0.5 * (cone.point[1:cone.U] - cone.point[(cone.U + 1):(2 * cone.U)])
        # @. tmp1 = ipwtj' * point_pq'
        # mul!(tmp4, tmp1, ipwtj)
        # mat .= tmp4
        ...
        lambdafact[j] = cholesky(Symmetric(lambda_plus, :L), Val(true), check = false)
        mat .= lambda_minus

        if !isposdef(lambdafact[j])
            @show "lambda plus fail"
            return false
        end
        ...
        for r in 3:(cone.R + 2)
            ...
            # li_lambda[r - 2] .= lambdafact[j] \ tmp4
            # mat -= tmp4 * li_lambda[r - 2]
            # uo += cone.U
            lambda = ipwtj' * Diagonal(point_pq) * ipwtj
            li_lambda[r - 2] .= lambda_plus \ lambda
            mat -= Symmetric(lambda * (lambda_plus \ lambda))
        end

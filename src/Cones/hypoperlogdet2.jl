...
mutable struct HypoPerLogdetUnsc{T <: HypReal} <: Cone{T}
    use_dual::Bool
    dim::Int
    side::Int
    ...
    function HypoPerLogdetUnsc{T}(dim::Int, is_dual::Bool) where {T <: HypReal}
        cone = new{T}()
...
HypoPerLogdetUnsc{T}(dim::Int) where {T <: HypReal} = HypoPerLogdetUnsc{T}(dim, false)

reset_data(cone::HypoPerLogdetUnsc) = (cone.feas_updated = cone.grad_updated = cone.hess_updated = cone.inv_hess_updated = cone.hess_prod_updated = cone.inv_hess_prod_updated = false)

function setup_data(cone::HypoPerLogdetUnsc{T}) where {T <: HypReal}
    reset_data(cone)
    dim = cone.dim
...
get_nu(cone::HypoPerLogdetUnsc) = cone.side + 2

function set_initial_point(arr::AbstractVector, cone::HypoPerLogdetUnsc)
    arr .= 0
    arr[1] = -1
    arr[2] = 1
    k = 3
...
# TODO remove allocs
function update_feas(cone::HypoPerLogdetUnsc)
...
function update_grad(cone::HypoPerLogdetUnsc)
    @assert cone.is_feas
    u = cone.point[1]
    v = cone.point[2]
...
function update_hess(cone::HypoPerLogdetUnsc)
...
function update_hess_prod(cone::HypoPerLogdetUnsc)
...
function hess_prod!(prod::AbstractVecOrMat, arr::AbstractVecOrMat, cone::HypoPerLogdetUnsc)

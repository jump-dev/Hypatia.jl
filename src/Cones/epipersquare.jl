...
reset_data(cone::EpiPerSquare) = (cone.feas_updated = cone.dual_feas_updated = cone.grad_updated = cone.dual_grad_updated =
    cone.hess_updated = cone.scal_hess_updated = cone.inv_hess_updated = cone.hess_sqrt_prod_updated = cone.inv_hess_sqrt_prod_updated = cone.nt_updated = false)

use_scaling(::EpiPerSquare) = true # TODO update oracles

use_correction(cone::EpiPerSquare) = false
...
function update_nt(cone::EpiPerSquare)
    ...
    # x = cone.point * sqrt(mu)
    # s = cone.dual_point
    # Q = Symmetric(Matrix{Float64}(-I, cone.dim, cone.dim))
    # Q[1, 1] = Q[2, 2] = 0
    # Q.data[1, 2] = 1
    # T = Symmetric(Matrix{Float64}(I, cone.dim, cone.dim))
    # T[1, 1] = T.data[1, 2] = inv(sqrt(2))
    # T[2, 2] = -inv(sqrt(2))
    # theta = sqrt(sqrt(dot(s, Q * s) / dot(x, Q * x)))
    # w = (inv(theta) * s + theta * Q * x) / sqrt(2) / sqrt(dot(x, s) + sqrt(x' * Q * x * s' * Q * s))
    # @show w ./ nt_point
    #
    # W = -Q + 2 * w * w'
    # W .*= abs2(theta)
    # # @show rotated_jdot(w, w)
    # # @show w ./ nt_point_sqrt
...
#
# # get_nu(cone::EpiPerSquare) = 2
# get_nu(cone::EpiPerSquare) = 1
#
# function set_initial_point(arr::AbstractVector, cone::EpiPerSquare{T}) where {T}
#     arr[1:2] .= sqrt(T(0.5))
#     arr[3:end] .= 0
#     return arr
# end
...
#         w = view(cone.dual_point, 3:cone.dim)
#         # cone.dual_dist = u * v - sum(abs2, w) / 2
#         cone.dual_dist = 2 * u * v - sum(abs2, w)
#         return cone.dual_dist > eps(T)
#     end
#     return false
# end
...
#
#     # mul!(H, cone.grad, cone.grad')
#     mul!(H, cone.grad, cone.grad', 2, false)
#     inv_dist = inv(cone.dist)
#     @inbounds for j in 3:cone.dim
#         H[j, j] += inv_dist
#     end
...
#         wj = @view arr[3:end, j]
#         # ga = (dot(w, wj) - v * uj - u * vj) / cone.dist
#         ga = 2 * (dot(w, wj) - v * uj - u * vj) / cone.dist
#         prod[1, j] = -ga * v - vj
#         prod[2, j] = -ga * u - uj
#         @. prod[3:end, j] = ga * w + wj
#     end
...
#     rtdist = cone.rtdist = sqrt(cone.dist)
#     # cone.denom = 2 * rtdist + cone.point[1] + cone.point[2]
#     cone.denom = sqrt(2) * rtdist + cone.point[1] + cone.point[2]
#     vec = cone.hess_sqrt_vec
#     @. @views vec[3:end] = cone.point[3:end] / (rtdist / sqrt(2))
#     # vec[1] = -cone.point[2] / rtdist - 1
#     # vec[2] = -cone.point[1] / rtdist - 1
#     vec[1] = -cone.point[2] / (rtdist / sqrt(2)) - 1
#     vec[2] = -cone.point[1] / (rtdist / sqrt(2)) - 1
#
#     cone.hess_sqrt_prod_updated = true
#     return
# end
...
#     rtdist = cone.rtdist = sqrt(cone.dist)
#     # cone.denom = 2 * rtdist + cone.point[1] + cone.point[2]
#     cone.denom = 2 * (rtdist / sqrt(2)) + cone.point[1] + cone.point[2]
#     vec = cone.inv_hess_sqrt_vec
#     copyto!(vec, cone.point)
#     # vec[1:2] .+= rtdist
#     vec[1:2] .+= rtdist / sqrt(2)
#
#     cone.inv_hess_sqrt_prod_updated = true
#     return
# end
...
#         @. prod[:, j] = dotj * vec
#     end
#     @. @views prod[1, :] -= arr[2, :] / (rtdist / sqrt(2))
#     @. @views prod[2, :] -= arr[1, :] / (rtdist / sqrt(2))
#     @. @views prod[3:end, :] += arr[3:end, :] / (rtdist / sqrt(2))
#     prod ./= sqrt(2) # new
#
#     return prod
# end
...
# end
#
# function update_nt(cone::EpiPerSquare, mu)
#     @assert cone.feas_updated
#     cone.dual_feas_updated || update_dual_feas(cone)
#     nt_point = cone.nt_point
#     nt_point_sqrt = cone.nt_point_sqrt

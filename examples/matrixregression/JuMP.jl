...
function build(inst::MatrixRegressionJuMP{T}) where {T <: Float64}
    ...
    if inst.nuc_obj
        JuMP.@constraint(
            model,
            vcat(loss, vec(loss_mat')) in MOI.NormNuclearCone(data_m, data_n)
        )
    else
        if data_n > data_p
        ...
        JuMP.@constraint(
            model,
            vcat(loss, 1, vec(loss_mat) / sqrt(data_n)) in JuMP.RotatedSecondOrderCone()
        )
    end
    ...
    if !iszero(inst.lam_fro)
        ...
        JuMP.@constraint(
            model,
            vcat(t_fro, inst.lam_fro * vec(A)) in JuMP.SecondOrderCone()
        )
        obj += t_fro
    end
    if !iszero(inst.lam_nuc)
        ...
        JuMP.@constraint(
            model,
            vcat(t_nuc, inst.lam_nuc * vec(A)) in MOI.NormNuclearCone(data_p, data_m)
        )
        obj += t_nuc
    end
    if !iszero(inst.lam_las)
        ...
        JuMP.@constraint(
            model,
            vcat(t_las, inst.lam_las * vec(A)) in MOI.NormOneCone(data_p * data_m + 1)
        )
        obj += t_las
    end
    if !iszero(inst.lam_glr)
        ...
        JuMP.@constraint(
            model,
            [i = 1:data_p],
            vcat(t_glr[i], inst.lam_glr * A[i, :]) in JuMP.SecondOrderCone()
        )
        obj += sum(t_glr)
    end
    if !iszero(inst.lam_glc)
        ...
        JuMP.@constraint(
            model,
            [i = 1:data_m],
            vcat(t_glc[i], inst.lam_glc * A[:, i]) in JuMP.SecondOrderCone()
        )
        obj += sum(t_glc)
    end

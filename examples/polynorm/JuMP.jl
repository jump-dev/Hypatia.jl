...
function build(inst::PolyNormJuMP{T}) where {T <: Float64}
    ...
    (U, pts, Ps, V, w) =
        PolyUtils.interpolate(dom, epi_halfdeg, calc_V = true, get_quadr = true)
    ...
    if inst.use_norm_cone
        ...
        JuMP.@constraint(model, vcat(f, vec(polys)) in cone{T}(num_polys + 1, U, Ps))
    elseif inst.use_l1
        lhs = one(T) * f
        ...
        JuMP.@constraint(model, lhs in Hypatia.WSOSInterpNonnegativeCone{T, T}(U, Ps))
    else
        R = num_polys + 1

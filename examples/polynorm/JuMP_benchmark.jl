...
polynorm_insts() = [
    [(n, d_factor * d, d, m, use_l1, use_norm_cone) for (n, d, m) in ndms]
    for ndms in polynorm_n_d_ms
    for d_factor in d_factors
    for use_l1 in use_l1s
    for use_norm_cone in use_norm_cone
    ]

insts = Dict()
insts["nat"] = (nothing, polynorm_insts())
return (PolyNormJuMP, insts)

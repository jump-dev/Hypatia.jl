...
function normfunction_data(; n::Int = 5, num_points::Int = 100)
    ...
    rmse = sqrt(sum(abs2.([y[i] - f(X[i,:]) for i in 1:num_points])) / num_points)
    return (X, y, n, rmse)
end
...
function run_synthetic()
    ...
    for n in 1:6
        ...
        (X, y, _, refrmse) = expfunction_data(n = n)
        folds = kfolds((X', y), k = 5)
        open(outfilename, "w") do f
            println(f, "# n = $n")
            println(f, "fold,refrmse,deg,use_wsos,tr_rmse,ts_rmse,time,status")
            foldcount = 0
            ...
            for ((Xtrain, ytrain), (Xtest, ytest)) in folds
                ...
                for deg in deg_options, use_wsos in wsos_options
                    println("running ", "deg = $deg, use_wsos = $use_wsos, n = $n")
                    ...
                    println(f, "$foldcount,$refrmse,$deg,$use_wsos,$tr_rmse,$ts_rmse,$s_tm,$(JuMP.termination_status(mdl))")
                end # model
            end # folds
        end # do
    end # n

...
function ShapeConRegrJuMP{Float64}(data_name::Symbol, args...)
...
function build(inst::ShapeConRegrJuMP{T}) where {T <: Float64}
    ...
    (U, points, Ps, V) = PolyUtils.interpolate(free_dom, halfdeg, calc_V = true)
    ...
    if inst.use_L1_obj || (num_points <= U)
        obj_K = (inst.use_L1_obj ? MOI.NormOneCone : MOI.SecondOrderCone)(1 + num_points)
        ...
        JuMP.@constraint(
            model,
            vcat(z, coef_R * vcat(regressor, 1)) in MOI.SecondOrderCone(2 + U)
        )
    end
    ...
    if inst.use_monotonicity
        ...
        (mono_U, mono_points, mono_Ps) = PolyUtils.interpolate(mono_dom, gradient_halfdeg)
        univ_chebs_der = [
            PolyUtils.calc_univariate_chebyshev(
                mono_points[:, i],
                2halfdeg,
                calc_gradient = true,
            ) for i in 1:n
        ...
        for j in 1:n
            ...
            V_g = PolyUtils.make_product_vandermonde(
                univ_chebs_g,
    ...
    if inst.use_convexity && !iszero(conv_profile)
        ...
        (conv_U, conv_points, conv_Ps) = PolyUtils.interpolate(conv_dom, hessian_halfdeg)
        univ_chebs_der = [
            PolyUtils.calc_univariate_chebyshev(
                conv_points[:, i],
                2halfdeg,
        ...
        for i in 1:n, j in 1:i
            ...
            V_H = PolyUtils.make_product_vandermonde(
                univ_chebs_H,
                PolyUtils.n_deg_exponents(n, 2halfdeg),
            )
            push!(V_Hs, V_H)
        ...
        H_interp = conv_profile * vcat([(F \ V_H')' * regressor for V_H in V_Hs]...)

        if inst.use_wsos
            if n == 1
                conv_K = Hypatia.WSOSInterpNonnegativeCone{T, T}(conv_U, conv_Ps)
            else
                Cones.scale_svec_incr!(H_interp, sqrt(T(2)), conv_U)
                ...
                coeffs_lhs = JuMP.@expression(
                    ...
                    sum(
                        sum(
                            conv_Ps[r][u, k] *
                            conv_Ps[r][u, l] *
                            psd_vars[r][(x1 - 1) * Ls[r] + k, (x2 - 1) * Ls[r] + l] for
                            k in 1:Ls[r] for l in 1:Ls[r]
                        ) for r in eachindex(Ls)
                    )
                )
                JuMP.@constraint(
                    model,
                    coeffs_lhs .== H_interp[conv_U .* (offset - 1) .+ (1:conv_U)]
                )
            end
        end
    end

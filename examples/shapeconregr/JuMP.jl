...
function shapeconregrJuMP(
    ...
    if use_wsos
        ...
        if !all(iszero, mono_profile)
            ...
            mono_wsos_cone = HYP.WSOSPolyInterpCone{Float64, Float64}(mono_U, [mono_P0, mono_PWts...])
            for j in 1:n
        ...
        if !iszero(conv_profile)
            ...
            conv_wsos_cone = HYP.WSOSPolyInterpMatCone{Float64}(n, conv_U, [conv_P0, conv_PWts...])

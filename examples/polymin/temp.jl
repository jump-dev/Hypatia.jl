...
function build_wsos_primal(dat)
    ...
    G = ones(dat.U, 1)
    h = dat.F_interp
    cones = CO.Cone{Float64}[CO.WSOSPolyInterp{Float64, eltype(first(dat.P_data))}(dat.U, dat.P_data, false)]
    cone_idxs = [1:dat.U]
...
function build_wsos_dual(dat; elim::Bool = true)
    ...
    cones = CO.Cone{Float64}[CO.WSOSPolyInterp{Float64, eltype(first(dat.P_data))}(dat.U, dat.P_data, true)]
    cone_idxs = [1:dat.U]
    return (c=c, A=A, b=b, G=G, h=h, cones=cones, cone_idxs=cone_idxs)
end
...
function build_psd_dual(dat)
    ...
    if eltype(dat.P_data[1]) <: Complex
        ...
        for Pk in dat.P_data
            ...
            push!(cones, CO.PosSemidef{Float64, ComplexF64}(dk))
...
function solve_model(dat, is_min::Bool)
    model = MO.PreprocessedLinearModel{Float64}(dat.c, dat.A, dat.b, dat.G, dat.h, dat.cones, dat.cone_idxs)
    solver = SO.HSDSolver{Float64}(model, verbose = true, tol_feas = 1e-5, tol_rel_opt = 1e-6, tol_abs_opt = 1e-5, max_iters = 200)
    SO.solve(solver)

...
function random_interp_data(
    ...
    n::Int,
    halfdeg::Int,
    dom = PolyUtils.Box{T}(-ones(T, n), ones(T, n)),
    ) where {T <: Real}
    (U, pts, Ps) = PolyUtils.interpolate(dom, halfdeg)
...
function real_poly_data(polyname::Symbol, T::Type{<:Real} = Float64)
    if polyname == :butcher
        ...
        dom = PolyUtils.Box{T}(T[-1,-0.1,-0.1,-1,-0.1,-0.1], T[0,0.9,0.5,-0.1,-0.05,-0.03])
    ...
    elseif polyname == :caprasse
        ...
        dom = PolyUtils.Box{T}(-T(0.5) * ones(T, 4), T(0.5) * ones(T, 4))
    ...
    elseif polyname == :goldsteinprice
        ...
        dom = PolyUtils.Box{T}(-2 * ones(T, 2), 2 * ones(T, 2))
        true_obj = 3
    # elseif polyname == :goldsteinprice_ball
    #     DP.@polyvar x[1:2]
    #     f = (1+(x[1]+x[2]+1)^2*(19-14x[1]+3x[1]^2-14x[2]+6x[1]*x[2]+3x[2]^2))*(30+(2x[1]-3x[2])^2*(18-32x[1]+12x[1]^2+48x[2]-36x[1]*x[2]+27x[2]^2))
    #     dom = PolyUtils.Ball{T}(zeros(T, 2), 2*sqrt(T(2)))
    ...
    #     dom = PolyUtils.Ellipsoid{T}(centers, Q)
    #     true_obj = 3 # small neighborhood around box
    ...
    #     D = T[1 0; 0 0.1]
    #     S = Q * D * Q
    #     dom = PolyUtils.Ellipsoid{T}(zeros(T, 2), S)
    #     true_obj = 0 # small neighborhood around box
    elseif polyname == :reactiondiffusion
        DP.@polyvar x[1:3]
        f = -x[1]+2x[2]-x[3]-0.835634534x[2]*(1+x[2])
        dom = PolyUtils.Box{T}(-5 * ones(T, 3), 5 * ones(T, 3))
    ...
    elseif polyname == :rosenbrock
        ...
        dom = PolyUtils.Box{T}(-5 * ones(T, 2), 10 * ones(T, 2))
        true_obj = 0
    # elseif polyname == :rosenbrock_ball
    #     DP.@polyvar x[1:2]
    #     f = (1-x[1])^2+100*(x[1]^2-x[2])^2
    #     dom = PolyUtils.Ball{T}(T(2.5) * ones(T, 2), T(7.5) * sqrt(T(2)))
    #     true_obj = 0 # small neighborhood around box
    elseif polyname == :schwefel
        DP.@polyvar x[1:3]
        f = (x[1]-x[2]^2)^2+(x[2]-1)^2+(x[1]-x[3]^2)^2+(x[3]-1)^2
        dom = PolyUtils.Box{T}(-10 * ones(T, 3), 10 * ones(T, 3))
        true_obj = 0
    # elseif polyname == :schwefel_ball
    #     DP.@polyvar x[1:3]
    #     f = (x[1]-x[2]^2)^2+(x[2]-1)^2+(x[1]-x[3]^2)^2+(x[3]-1)^2
    #     dom = PolyUtils.Ball{T}(zeros(T, 3), 10 * sqrt(T(3)))
    #     true_obj = 0 # small neighborhood around box
    else

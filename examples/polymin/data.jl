...
function real_poly_data(polyname::Symbol, T::DataType = Float64)
    if polyname == :butcher
        ...
        dom = MU.Box{T}(T[-1,-0.1,-0.1,-1,-0.1,-0.1], T[0,0.9,0.5,-0.1,-0.05,-0.03])
    ...
    elseif polyname == :caprasse
        ...
        dom = MU.Box{T}(-T(0.5) * ones(T, 4), T(0.5) * ones(T, 4))
    ...
    elseif polyname == :goldsteinprice
        ...
        dom = MU.Box{T}(-2 * ones(T, 2), 2 * ones(T, 2))
        true_obj = 3
    elseif polyname == :goldsteinprice_ball
        DP.@polyvar x[1:2]
        f = (1+(x[1]+x[2]+1)^2*(19-14x[1]+3x[1]^2-14x[2]+6x[1]*x[2]+3x[2]^2))*(30+(2x[1]-3x[2])^2*(18-32x[1]+12x[1]^2+48x[2]-36x[1]*x[2]+27x[2]^2))
        dom = MU.Ball{T}(zeros(T, 2), 2*sqrt(T(2)))
    ...
    elseif polyname == :rosenbrock
        ...
        dom = MU.Box{T}(-5 * ones(T, 2), 10 * ones(T, 2))
        true_obj = 0
    ...
    elseif polyname == :schwefel
        ...
        dom = MU.Box{T}(-10 * ones(T, 3), 10 * ones(T, 3))
        true_obj = 0
    elseif polyname == :schwefel_ball
        DP.@polyvar x[1:3]
        f = (x[1]-x[2]^2)^2+(x[2]-1)^2+(x[1]-x[3]^2)^2+(x[3]-1)^2
        dom = MU.Ball{T}(zeros(T, 3), 10 * sqrt(T(3)))
        true_obj = 0 # small neighborhood around box
    else

...
polys = Dict{Symbol, NamedTuple}(
    ...
    :magnetism7 => (n=7, lbs=-ones(7), ubs=ones(7), deg=4,
...
hard_polys = Dict{Symbol, NamedTuple}(
    :butcher => (n=6, lbs=[-1.0,-0.1,-0.1,-1.0,-0.1,-0.1], ubs=[0.0,0.9,0.5,-0.1,-0.05,-0.03], deg=3,
        fn=((u,v,w,x,y,z) -> z*v^2+y*w^2-u*x^2+x^3+x^2-(1/3)*u+(4/3)*x)
        ),
    :caprasse => (n=4, lbs=-0.5 * ones(4), ubs=0.5 * ones(4), deg=4, # changed from 8
    ...
    :motzkin => (n=2, lbs=[0.4, -10.0], ubs=[0.6, 10.0], deg=6, # min at pm 0.5
        fn=((x,y) -> 1-48x^2*y^2+64x^2*y^4+64x^4*y^2)
        ),
    :robinson => (n=2, lbs=[-1, -10], ubs=[1, 10], deg=6,
        fn=((x,y) -> 1+x^6+y^6-x^4*y^2+x^4-x^2*y^4+y^4-x^2+y^2+3x^2*y^2) # min at pm (1/srt(3), 0)
        ),

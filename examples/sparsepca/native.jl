...
#         vcat(1, [X[i, j] * (i == j ? 1 : 2) for i in 1:n for j in 1:i]) in CO.EpiNormInf{Float64}(1 + div(n * (n + 1), 2), true)
...
#     return (model = model,)
# end
function sparsepca(n::Int, k::Int; T = Float64)
    dimx = div(n * (n + 1), 2)
    c = randn(dimx)
    b = T[10]
    A = zeros(T, 1, dimx)

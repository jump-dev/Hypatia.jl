...
function DensityEstJuMP{Float64}(
    ...
    (num_obs, n) = size(X)
    true_obj = NaN
    return DensityEstJuMP{Float64}(dataset_name, num_obs, n, X, true_obj, deg, use_monomial_space, use_wsos)
end
function DensityEstJuMP{Float64}(
    density_name::Symbol,
    use_monomial_space::Bool,
    ...
    return DensityEstJuMP{Float64}(density_name, num_obs, n, X, true_obj, deg, use_monomial_space, use_wsos)
end
function DensityEstJuMP{Float64}(
    num_obs::Int,
    n::Int,
    args...)
    X = randn(num_obs, n)
    return DensityEstJuMP{Float64}(:random, num_obs, n, X, NaN, args...)
end
...
example_tests(::Type{DensityEstJuMP{Float64}}, ::FastInstances) = begin
    ...
    ((:density2, true, true), nothing, options),
    ((:density3, true, true), nothing, options),
    ]
end
